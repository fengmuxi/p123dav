...
class P123FileSystemProvider(DAVProvider):
    ...
    def __init__(
        ...
        ttl: float = 10, 
        refresh: bool = False, 
        token_path: str = "./config/config.txt",
    ):
        super().__init__()
    ...
    def refresh_token(self, /) -> bool:
        """刷新token，如果token失效则重新获取"""
        try:
            # 尝试调用一个简单的API来验证token
            self.client.user_info()
            return True
        except Exception as e:
            if "token is expired" in str(e).lower() or "401" in str(e):
                print(f"检测到token过期：{e}，尝试重新获取...")
                try:
                    # 使用用户名密码重新获取token
                    if self.username and self.password:
                        new_client = P123Client(self.username, self.password)
                        # 更新client的token
                        self.client.token = new_client.token
                        # 持久化新token
                        with open(self.token_path, "w", encoding="utf-8") as f:
                            f.write(new_client.token)
                        print("token已成功刷新")
                        return True
                    else:
                        print("无法刷新token：缺少用户名和密码")
                        return False
                except Exception as refresh_e:
                    print(f"刷新token失败：{refresh_e}")
                    return False
            return True
